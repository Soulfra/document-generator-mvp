...
exports.aiLaboratoryRouter = void 0;
const express_1 = require("express");
const zod_1 = require("zod");
const logger_1 = require("../../utils/logger");
const profile_service_1 = require("../../services/profile.service");
const router_1 = require("../../llm/router");
const storytelling_service_1 = require("../../services/storytelling/storytelling.service");
const archetype_service_1 = require("../../services/soulfra/archetype.service");
const router = (0, express_1.Router)();
exports.aiLaboratoryRouter = router;
const testPromptSchema = zod_1.z.object({
    question: zod_1.z.string().min(1).max(2000),
    profiles: zod_1.z.array(zod_1.z.string()).min(1).max(5),
    storyMode: zod_1.z.boolean().optional().default(false),
...
const feedbackSchema = zod_1.z.object({
    ...
    rating: zod_1.z.enum(['up', 'down', 'love', 'hate']),
...
router.post('/test-prompt', async (req, res) => {
    try {
        ...
        logger_1.logger.info('AI Laboratory prompt test started', {
            sessionId,
            profileCount: profiles.length,
            storyMode
        });
        const responses = await Promise.allSettled(profiles.map(async (profileId) => {
            let profile = LABORATORY_PROFILES[profileId];
            if (!profile) {
                profile = await profile_service_1.profileService.getProfile(profileId);
            }
            if (!profile) {
            ...
            if (isSoulfrArchetype) {
                try {
                    archetypeResponse = await archetype_service_1.soulfrArchetypeService.generateArchetypeResponse(profileId, question, { storyMode, sessionId });
                    logger_1.logger.info('Soulfra archetype response generated', {
                        profileId,
                        resonance: archetypeResponse.resonance
                    });
                }
                catch (error) {
                    logger_1.logger.warn('Soulfra archetype failed, using regular profile', {
                        profileId,
                        error: error.message
                    });
                }
            ...
            let response;
            try {
                response = await router_1.llmRouter.generateResponse({
                ...
                logger_1.logger.warn('LLM router unavailable, using mock response', { profileId, error: error.message });
                response = {
            ...
            if (archetypeResponse) {
                ...
                try {
                    const storyEnhancement = await storytelling_service_1.storytellingService.enhanceWithStory(question, response.content, {
                    ...
                    logger_1.logger.info('Story enhancement applied', {
                        profileId,
                        memorabilityScore: storyEnhancement.memorabilityScore
                    });
                }
                catch (error) {
                    logger_1.logger.warn('Story enhancement failed, using original response', {
                        profileId,
                        error: error.message
                    });
                }
        ...
        if (userId && sessionId) {
            logger_1.logger.info('Storing laboratory interaction for learning', {
                userId,
                sessionId,
        ...
        logger_1.logger.error('AI Laboratory test failed', { error, body: req.body });
        res.status(500).json({
            success: false,
            error: {
...
router.post('/feedback', async (req, res) => {
    try {
        ...
        logger_1.logger.info('Laboratory feedback received', feedbackRecord);
        ...
        logger_1.logger.error('Laboratory feedback failed', { error, body: req.body });
        res.status(500).json({
            success: false,
            error: {
...
router.post('/qr-session', async (req, res) => {
    try {
        ...
        logger_1.logger.info('Laboratory QR session created', { sessionId });
        res.json({
            success: true,
            data: {
                sessionId,
        ...
        logger_1.logger.error('QR session creation failed', { error });
        res.status(500).json({
            success: false,
            error: {
...
router.post('/archetype/blessing', async (req, res) => {
    try {
        ...
        const blessing = await archetype_service_1.soulfrArchetypeService.performBlessingCeremony(archetypeId);
        logger_1.logger.info('Blessing ceremony performed', { archetypeId });
        res.json({
            success: true,
            data: {
                archetypeId,
        ...
        logger_1.logger.error('Blessing ceremony failed', { error, body: req.body });
        res.status(400).json({
            success: false,
            error: error.message
        });
...
router.get('/analytics', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Laboratory analytics failed', { error });
        res.status(500).json({
            success: false,
            error: {
...
//# sourceMappingURL=ai-laboratory.route.js.map