...
router.get('/services', async (req, res) => {
    try {
        logger_1.logger.info('Fetching service catalog');
        ...
        logger_1.logger.error('Failed to fetch services', { error });
...
router.get('/jobs', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Failed to fetch jobs', { error });
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const [totalJobs, completedJobs, totalSpent] = await Promise.all([
            ...
            database_1.prisma.job.count({ where: { userId, status: 'COMPLETED' } }),
...
async function getServicesUsed(userId) {
    const jobs = await database_1.prisma.job.findMany({
        where: { userId },
        select: { type: true },
        distinct: ['type']
...
router.post('/bundles', async (req, res) => {
    try {
        ...
        logger_1.logger.info('Bundle created', { bundleId: bundle.id, services: bundleDefinition.services });
        res.json({
            success: true,
            data: {
                bundleId: bundle.id,
        ...
        logger_1.logger.error('Failed to create bundle', { error });
...
router.post('/jobs', async (req, res) => {
    try {
        ...
        logger_1.logger.info('Job created', { jobId: job.id, type });
        res.json({
            success: true,
            data: {
                id: job.id,
...
router.get('/jobs/:jobId/download', async (req, res) => {
    try {
        ...
        const { generatePresignedUrl } = await Promise.resolve().then(() => __importStar(require('../../utils/storage')));
