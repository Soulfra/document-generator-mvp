...
exports.processLoadTesting = processLoadTesting;
 ...
 */
async function processLoadTesting(jobId, config, progressCallback) {
    ...
    try {
        ...
        logger_1.logger.info('Starting loadTesting processing', { jobId });
        // Download and process
        const inputBuffer = await (0, storage_1.downloadFromS3)(job.inputFileUrl);
        const tempDir = path_1.default.join('/tmp', `loadTesting-${jobId}`);
        ...
        const result = await new Promise((resolve, reject) => {
            const proc = (0, child_process_1.spawn)('python3', [
                ...
                path_1.default.join(__dirname, '../soulfra-scripts/load-testing.py'),
                '--input', inputPath,
                '--output', tempDir,
                '--job-mode'
            ]);
        ...
        const outputUrl = await (0, storage_1.uploadToS3)(outputBuffer, `loadTesting/${jobId}/output.zip`);
        ...
        logger_1.logger.error('loadTesting processing failed', { jobId, error });
        throw error;
    }
}
//# sourceMappingURL=loadTesting.service.js.map