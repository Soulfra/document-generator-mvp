 ...
 */
export async function processChatApi(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting chatApi processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `chatApi-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/chat-api.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `chatApi/${jobId}/output.zip`);
    ...
    logger.error('chatApi processing failed', { jobId, error });
    throw error;
  }
}
