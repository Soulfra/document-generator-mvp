 ...
 */
export async function processDocumentationGenerator(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting documentationGenerator processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `documentationGenerator-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/documentation-generator.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `documentationGenerator/${jobId}/output.zip`);
    ...
    logger.error('documentationGenerator processing failed', { jobId, error });
    throw error;
  }
}
