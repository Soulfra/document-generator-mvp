 ...
 */
export async function processAiUx(
  jobId: string,
  config: AiUxConfig,
  ...
  try {
    ...
    logger.info('Starting aiUx processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `aiUx-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/ai-ux.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `aiUx/${jobId}/output.zip`);
    ...
    logger.error('aiUx processing failed', { jobId, error });
    throw error;
  }
}
