 ...
 */
export async function processAiVersioning(
  jobId: string,
  config: AiVersioningConfig,
  progressCallback?: (progress: number) => void
): Promise<AiVersioningResult> {
  ...
  try {
    ...
    logger.info('Starting aiVersioning processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `aiVersioning-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/ai-versioning.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `aiVersioning/${jobId}/output.zip`);
    ...
    logger.error('aiVersioning processing failed', { jobId, error });
    throw error;
  }
}
