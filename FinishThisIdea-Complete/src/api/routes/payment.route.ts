...
const createTieredPaymentSchema = z.object({
  tierId: z.enum(['cleanup', 'developer', 'team', 'enterprise']),
...
// ============================================================================
// 💰 PRICING DISPLAY & RECOMMENDATIONS (NEW)
// ============================================================================

/**
 * GET /api/payment/pricing
 * Get all pricing tiers with personalized recommendations
 */
router.get(
  '/pricing',
  asyncHandler(async (req, res) => {
    ...
    const pricingDisplay = await pricingService.getPricingDisplay(userId);
    
    // Track pricing page view
    if (userId) {
      await presenceLogger.logUserPresence('pricing_viewed', {
        userId,
        metadata: {
...
/**
 * POST /api/payment/create-tiered-session
 * Create a checkout session for any pricing tier
 */
router.post(
  '/create-tiered-session',
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          price: session.price,
          tier: PRICING_TIERS[session.tierId]
        }
      });

    } catch (error) {
      logger.error('Failed to create tiered checkout session', { error, data, userId });
      throw new AppError('Failed to create payment session', 500, 'PAYMENT_SESSION_FAILED');
    }
  })
);

// ============================================================================
// 📊 ORIGINAL PAYMENT ENDPOINTS (ENHANCED)
// ============================================================================
