 ...
 */
export async function processOrchestrationApi(
  jobId: string,
  config: OrchestrationApiConfig,
  progressCallback?: (progress: number) => void
): Promise<OrchestrationApiResult> {
  ...
  try {
    ...
    logger.info('Starting orchestrationApi processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `orchestrationApi-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/orchestration-api.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `orchestrationApi/${jobId}/output.zip`);
    ...
    logger.error('orchestrationApi processing failed', { jobId, error });
    throw error;
  }
}
