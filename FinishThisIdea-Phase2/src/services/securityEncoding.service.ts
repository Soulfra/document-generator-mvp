 ...
 */
export async function processSecurityEncoding(
  jobId: string,
  config: SecurityEncodingConfig,
  progressCallback?: (progress: number) => void
): Promise<SecurityEncodingResult> {
  ...
  try {
    ...
    logger.info('Starting securityEncoding processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `securityEncoding-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/security-encoding.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `securityEncoding/${jobId}/output.zip`);
    ...
    logger.error('securityEncoding processing failed', { jobId, error });
    throw error;
  }
}
