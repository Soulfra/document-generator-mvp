...
exports.processAiUx = processAiUx;
 ...
 */
async function processAiUx(jobId, config, progressCallback) {
    ...
    try {
        ...
        logger_1.logger.info('Starting aiUx processing', { jobId });
        // Download and process
        const inputBuffer = await (0, storage_1.downloadFromS3)(job.inputFileUrl);
        const tempDir = path_1.default.join('/tmp', `aiUx-${jobId}`);
        ...
        const result = await new Promise((resolve, reject) => {
            const proc = (0, child_process_1.spawn)('python3', [
                ...
                path_1.default.join(__dirname, '../soulfra-scripts/ai-ux.py'),
                '--input', inputPath,
                '--output', tempDir,
                '--job-mode'
            ]);
        ...
        const outputUrl = await (0, storage_1.uploadToS3)(outputBuffer, `aiUx/${jobId}/output.zip`);
        ...
        logger_1.logger.error('aiUx processing failed', { jobId, error });
        throw error;
    }
}
//# sourceMappingURL=aiUx.service.js.map