 ...
 */
export async function processIdeaExtraction(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting ideaExtraction processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `ideaExtraction-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/idea-extraction.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `ideaExtraction/${jobId}/output.zip`);
    ...
    logger.error('ideaExtraction processing failed', { jobId, error });
    throw error;
  }
}
