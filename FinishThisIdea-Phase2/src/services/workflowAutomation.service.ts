 ...
 */
export async function processWorkflowAutomation(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting workflowAutomation processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `workflowAutomation-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/workflow-automation.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `workflowAutomation/${jobId}/output.zip`);
    ...
    logger.error('workflowAutomation processing failed', { jobId, error });
    throw error;
  }
}
