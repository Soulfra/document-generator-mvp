...
exports.processAiConductor = processAiConductor;
 ...
 */
async function processAiConductor(jobId, config, progressCallback) {
    ...
    try {
        ...
        logger_1.logger.info('Starting aiConductor processing', { jobId });
        // Download and process
        const inputBuffer = await (0, storage_1.downloadFromS3)(job.inputFileUrl);
        const tempDir = path_1.default.join('/tmp', `aiConductor-${jobId}`);
        ...
        const result = await new Promise((resolve, reject) => {
            const proc = (0, child_process_1.spawn)('python3', [
                ...
                path_1.default.join(__dirname, '../soulfra-scripts/ai-conductor.py'),
                '--input', inputPath,
                '--output', tempDir,
                '--job-mode'
            ]);
        ...
        const outputUrl = await (0, storage_1.uploadToS3)(outputBuffer, `aiConductor/${jobId}/output.zip`);
        ...
        logger_1.logger.error('aiConductor processing failed', { jobId, error });
        throw error;
    }
}
//# sourceMappingURL=aiConductor.service.js.map