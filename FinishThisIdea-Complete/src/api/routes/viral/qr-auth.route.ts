...
import { qrAuthService } from '../../services/viral/qr-auth.service';
...
/**
 * POST /api/qr-auth/laboratory/session
...
router.post('/laboratory/session', async (req, res) => {
  try {
    ...
    const qrCode = await qrAuthService.generateLabSessionQR(
      sessionId, 
      profiles || ['storyteller', 'technical', 'educator'], 
      storyMode || false
    );

    // Log laboratory session QR generation
    await presenceLogger.logUserPresence('qr_lab_session', {
      sessionId: req.sessionID,
      userId: req.body.userId || 'anonymous',
      userAgent: req.get('User-Agent'),
      ipAddress: req.ip,
    ...
    logger.error('Failed to generate laboratory session QR', { error });
    res.status(500).json({
      success: false,
...
router.post('/offline/sync', async (req, res) => {
  try {
    ...
    const qrCode = await qrAuthService.generateOfflineSyncQR(userId, syncData);

    // Log offline sync QR generation
    await presenceLogger.logUserPresence('qr_offline_sync', {
      sessionId: req.sessionID,
      userId: userId,
      userAgent: req.get('User-Agent'),
      ipAddress: req.ip,
    ...
    logger.error('Failed to generate offline sync QR', { error });
    res.status(500).json({
      success: false,
...
/**
 * POST /api/qr-auth/story/activate
...
router.post('/story/activate', async (req, res) => {
  try {
    ...
    const qrCode = await qrAuthService.generateStoryModeQR(sessionId, storyPreferences);

    // Log story mode QR generation
    await presenceLogger.logUserPresence('qr_story_mode', {
      sessionId: req.sessionID,
      userId: req.body.userId || 'anonymous',
      userAgent: req.get('User-Agent'),
      ipAddress: req.ip,
    ...
    logger.error('Failed to generate story mode QR', { error });
    res.status(500).json({
      success: false,
...
/**
 * POST /api/qr-auth/team/collaborate
...
router.post('/team/collaborate', async (req, res) => {
  try {
    ...
    const qrCode = await qrAuthService.generateTeamCollabQR(teamId, permissions, aiProfiles);

    // Log team collaboration QR generation
    await presenceLogger.logUserPresence('qr_team_collab', {
      sessionId: req.sessionID,
      userId: req.body.userId || 'anonymous',
      userAgent: req.get('User-Agent'),
      ipAddress: req.ip,
    ...
    logger.error('Failed to generate team collaboration QR', { error });
    res.status(500).json({
      success: false,
