 ...
 */
export async function processCodeConsolidation(
  jobId: string,
  config: CodeConsolidationConfig,
  progressCallback?: (progress: number) => void
): Promise<CodeConsolidationResult> {
  ...
  try {
    ...
    logger.info('Starting codeConsolidation processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `codeConsolidation-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/code-consolidation.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `codeConsolidation/${jobId}/output.zip`);
    ...
    logger.error('codeConsolidation processing failed', { jobId, error });
    throw error;
  }
}
