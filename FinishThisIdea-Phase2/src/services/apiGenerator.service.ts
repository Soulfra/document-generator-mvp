 ...
 */
export async function processApiGenerator(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting apiGenerator processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `apiGenerator-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/api-generator.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `apiGenerator/${jobId}/output.zip`);
    ...
    logger.error('apiGenerator processing failed', { jobId, error });
    throw error;
  }
}
