...
exports.processDistributedSystem = processDistributedSystem;
 ...
 */
async function processDistributedSystem(jobId, config, progressCallback) {
    ...
    try {
        ...
        logger_1.logger.info('Starting distributedSystem processing', { jobId });
        // Download and process
        const inputBuffer = await (0, storage_1.downloadFromS3)(job.inputFileUrl);
        const tempDir = path_1.default.join('/tmp', `distributedSystem-${jobId}`);
        ...
        const result = await new Promise((resolve, reject) => {
            const proc = (0, child_process_1.spawn)('python3', [
                ...
                path_1.default.join(__dirname, '../soulfra-scripts/distributed-system.py'),
                '--input', inputPath,
                '--output', tempDir,
                '--job-mode'
            ]);
        ...
        const outputUrl = await (0, storage_1.uploadToS3)(outputBuffer, `distributedSystem/${jobId}/output.zip`);
        ...
        logger_1.logger.error('distributedSystem processing failed', { jobId, error });
        throw error;
    }
}
//# sourceMappingURL=distributedSystem.service.js.map