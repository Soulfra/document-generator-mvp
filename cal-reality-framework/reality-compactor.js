...
/**
 * 🌀 CONVERSATION COMPACTOR SYSTEM
 * 
 * Compacts conversations, removes duplicates, deduplicates files,
 * and creates efficient summaries for the Cal Reality Framework system.
 */
...
class RealityCompactor extends EventEmitter {
    constructor() {
        super();
        ...
        console.log('🌀 Reality Compactor initialized');
    }
    ...
    async compact(inputPath, outputPath = null) {
        console.log(`\n🌀 Starting compaction of: ${inputPath}`);
        ...
        if (stats.type === 'conversation') {
            return await this.compactRealitys(inputPath, outputPath);
     ...
     */
    async compactRealitys(inputPath, outputPath) {
        console.log('\n💬 Compacting conversations...');
        
        const conversations = await this.loadRealitys(inputPath);
        const compacted = [];
        ...
        if (outputPath) {
            await this.saveCompactedRealitys(compacted, outputPath);
        }
    ...
    async generateSummary(messages) {
        ...
        let summary = `📝 **Reality Summary** (${totalMessages} messages)\n\n`;
     ...
     */
    async loadRealitys(inputPath) {
        const stats = await fs.stat(inputPath);
        
        if (stats.isFile()) {
            return [await this.loadSingleReality(inputPath)];
        } else {
            return await this.loadMultipleRealitys(inputPath);
        }
    }
     ...
     */
    async loadSingleReality(filePath) {
        ...
        if (ext === '.json') {
            ...
            return this.parseTextReality(content, filePath);
        }
    }
     ...
     */
    async loadMultipleRealitys(dirPath) {
        const files = await this.getAllFiles(dirPath);
        const conversations = [];
        
        for (const file of files) {
            if (this.isRealityFile(file)) {
                try {
                    const conversation = await this.loadSingleReality(file);
     ...
     */
    parseTextReality(content, filePath) {
        const lines = content.split('\n');
        const messages = [];
     ...
     * 💾 Save compacted conversations
     */
    async saveCompactedRealitys(conversations, outputPath) {
        ...
        const report = {
            timestamp: new Date().toISOString(),
            totalRealitys: conversations.length,
    ...
    async compactEverything(inputPath, outputPath) {
        ...
        const results = {
            conversations: await this.compactRealitys(inputPath, outputPath),
        ...
        console.log('\n✅ Compaction complete!');
        console.log(`💬 Realitys: ${overallReport.conversations.count} processed`);
...
if (require.main === module) {
    async function main() {
        const compactor = new RealityCompactor();
        ...
        console.log('🌀 Starting Cal Reality Framework Reality Compactor...');
...
module.exports = RealityCompactor;