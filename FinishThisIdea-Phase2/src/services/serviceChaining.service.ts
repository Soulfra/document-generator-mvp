 ...
 */
export async function processServiceChaining(
  jobId: string,
  config: ServiceChainingConfig,
  progressCallback?: (progress: number) => void
): Promise<ServiceChainingResult> {
  ...
  try {
    ...
    logger.info('Starting serviceChaining processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `serviceChaining-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/service-chaining.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `serviceChaining/${jobId}/output.zip`);
    ...
    logger.error('serviceChaining processing failed', { jobId, error });
    throw error;
  }
}
