...
// Storytelling Service - Adapted from your storytelling-soul-engine
// Transforms technical responses into engaging narratives
...
export interface StoryContext {
  ...
  domain?: string; // e-commerce, auth, database, etc.
}
...
export class StorytellingService {
  private narrativeTemplates: Map<string, any>;
  private characterBank: Map<string, any>;
  private metaphorLibrary: Map<string, any>;
  ...
  private initializeStorytellingAssets() {
    // Initialize narrative templates for different domains
  ...
  async createStoryResponse(context: StoryContext): Promise<StoryResponse> {
    try {
      logger.info('Creating story response', { domain: context.domain });

      // Determine the best narrative style for this context
      ...
      // Extract story elements for metadata
      ...
      logger.info('Story response created', { 
      ...
      logger.error('Failed to create story response', { error, context });
      throw new Error('Story generation failed');
    }
  }

  private selectNarrativeStyle(context: StoryContext): string {
    // Prefer user's style if specified
    if (context.userProfile?.preferredStyle) {
      return context.userProfile.preferredStyle;
    }

    // Select based on domain and complexity
    const domain = context.domain || 'general';
    const complexity = context.userProfile?.complexityLevel || 'intermediate';

    if (domain === 'e-commerce') return 'adventure';
    if (domain === 'authentication') return 'mystery';
    if (domain === 'database') return 'modern';
    if (complexity === 'beginner') return 'fairy-tale';
    
    return 'adventure'; // Default
  }

  private extractTechnicalConcepts(technicalResponse: string): string[] {
  ...
  private mapConceptsToStory(concepts: string[], domain: string): any {
    const mapping = {
      conceptMap: {} as Record<string, string>,
      characters: [] as any[],
      setting: '',
      conflict: '',
      resolution: ''
    ...
    // Map each technical concept to a story element
    concepts.forEach(concept => {
      ...
      if (character) {
        ...
        // Generate a metaphor for unmapped concepts
        mapping.conceptMap[concept] = this.generateMetaphorFor(concept);
      }
    });

    // Set narrative context based on domain
  ...
  private generateMetaphorFor(concept: string): string {
    const metaphors = {
  ...
  private generateOpening(context: StoryContext, style: string, storyMapping: any): string {
    const templates = {
  ...
  private generateJourney(context: StoryContext, storyMapping: any): string {
  ...
  private generateResolution(context: StoryContext, storyMapping: any): string {
  ...
  private generateWisdom(context: StoryContext): string {
  ...
  private getMainCharacter(storyMapping: any): string {
  ...
  private extractCharacters(storyMapping: any): string[] {
    return storyMapping.characters.map(c => c.name);
  }

  private extractMetaphors(storyMapping: any): string[] {
    return Object.values(storyMapping.conceptMap) as string[];
  }

  private extractMoral(context: StoryContext, narrative: string): string {
    // Extract the moral/lesson from the narrative
  ...
  private calculateMemorabilityScore(narrative: string, storyMapping: any): number {
    let score = 5.0; // Base score

    // Award points for story elements
    if (narrative.includes('Once upon a time') || narrative.includes('In a land')) score += 1.0;
    if (storyMapping.characters.length > 2) score += 0.5;
    if (narrative.includes('🏰') || narrative.includes('✨') || narrative.includes('🌟')) score += 0.5;
    if (narrative.length > 200 && narrative.length < 800) score += 1.0; // Good length
    if (narrative.includes('moral') || narrative.includes('wisdom')) score += 0.5;
  ...
  // Public method to enhance any technical response with storytelling
  async enhanceWithStory(
    question: string,
    technicalResponse: string,
    userPreferences?: any
