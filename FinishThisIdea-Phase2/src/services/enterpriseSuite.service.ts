 ...
 */
export async function processEnterpriseSuite(
  jobId: string,
  config: EnterpriseSuiteConfig,
  progressCallback?: (progress: number) => void
): Promise<EnterpriseSuiteResult> {
  ...
  try {
    ...
    logger.info('Starting enterpriseSuite processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `enterpriseSuite-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/enterprise-suite.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `enterpriseSuite/${jobId}/output.zip`);
    ...
    logger.error('enterpriseSuite processing failed', { jobId, error });
    throw error;
  }
}
