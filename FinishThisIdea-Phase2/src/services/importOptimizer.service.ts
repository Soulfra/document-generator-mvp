 ...
 */
export async function processImportOptimizer(
  jobId: string,
  config: ImportOptimizerConfig,
  progressCallback?: (progress: number) => void
): Promise<ImportOptimizerResult> {
  ...
  try {
    ...
    logger.info('Starting importOptimizer processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `importOptimizer-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/import-optimizer.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `importOptimizer/${jobId}/output.zip`);
    ...
    logger.error('importOptimizer processing failed', { jobId, error });
    throw error;
  }
}
