 ...
 */
export async function processContainerization(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting containerization processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `containerization-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/containerization.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `containerization/${jobId}/output.zip`);
    ...
    logger.error('containerization processing failed', { jobId, error });
    throw error;
  }
}
