 ...
 */
export async function processUxOptimizer(
  jobId: string,
  config: UxOptimizerConfig,
  progressCallback?: (progress: number) => void
): Promise<UxOptimizerResult> {
  ...
  try {
    ...
    logger.info('Starting uxOptimizer processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `uxOptimizer-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/ux-optimizer.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `uxOptimizer/${jobId}/output.zip`);
    ...
    logger.error('uxOptimizer processing failed', { jobId, error });
    throw error;
  }
}
