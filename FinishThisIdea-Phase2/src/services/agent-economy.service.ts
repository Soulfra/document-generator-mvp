...
class AgentEconomyService {
  ...
  private initializeAgentTemplates(): void {
    ...
    logger.info('Agent templates initialized', { count: templates.length });
  }

  /**
   * Mint a new AI agent for a user (viral growth mechanic)
   */
  async mintAgent(userId: string, templateId: string, customName?: string, metadata: any = {}): Promise<UserAgent> {
    try {
      ...
      logger.info('Agent minting requested', { userId, templateId, mintCost });
      ...
      // Deduct minting cost
      await treasuryService.awardAchievementTokens(userId, 'agent_minting_cost', -mintCost);

      // Award agent creation achievement
      await treasuryService.awardAchievementTokens(userId, 'agent_minted', 50);
      ...
      logger.info('Agent minted successfully', {
        agentId,
        userId,
        templateId,
        mintCost
      ...
      logger.error('Error minting agent', { error: error.message, userId, templateId });
      throw error;
    }
  }
   ...
   */
  async startCollaboration(projectId: string, participantAgents: { agentId: string; role: string }[]): Promise<AgentCollaboration> {
    try {
      const collaborationId = `collab-${uuidv4()}`;
      
      const participants = await Promise.all(
        participantAgents.map(async ({ agentId, role }) => {
      ...
      for (const participant of participants) {
        await treasuryService.awardAchievementTokens(participant.ownerId, 'agent_collaboration', 25);
      ...
      await presenceLogger.logUserPresence('agent_collaboration_started', {
        userId: 'system',
        sessionId: 'system',
        metadata: {
          collaborationId,
      ...
      logger.error('Error starting agent collaboration', { error: error.message, projectId });
      throw error;
    }
  }
   ...
   */
  async completeCollaboration(collaborationId: string, outcome: 'success' | 'partial' | 'failed', revenueGenerated: number = 0): Promise<void> {
    try {
      ...
      for (const participant of collaboration.participants) {
        ...
        await treasuryService.awardAchievementTokens(participant.ownerId, 'collaboration_completed', totalReward);
      ...
      await presenceLogger.logUserPresence('agent_collaboration_completed', {
        userId: 'system',
        sessionId: 'system',
        metadata: {
          collaborationId,
      ...
      logger.error('Error completing agent collaboration', { error: error.message, collaborationId });
      throw error;
    }
  }
  ...
  async getMarketplaceStats(): Promise<any> {
    try {
      ...
      const agentsByTemplate = new Map<string, number>();
      ...
      logger.error('Error getting marketplace stats', { error: error.message });
      return {
        totalAgents: 0,
        publicAgents: 0,
   ...
   */
  async makeAgentPublic(agentId: string, userId: string): Promise<void> {
    try {
      const agent = this.userAgents.get(agentId);
      if (!agent) {
        throw new Error(`Agent ${agentId} not found`);
      ...
      await treasuryService.awardAchievementTokens(userId, 'agent_made_public', 20);

      await presenceLogger.logUserPresence('agent_made_public', {
        userId,
        sessionId: 'system',
        metadata: { agentId, agentName: agent.customName }
      });

      logger.info('Agent made public', { agentId, userId });

    } catch (error) {
      logger.error('Error making agent public', { error: error.message, agentId, userId });
      throw error;
    }
  }
...
export const agentEconomyService = new AgentEconomyService();