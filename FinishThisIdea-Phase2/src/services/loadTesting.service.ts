 ...
 */
export async function processLoadTesting(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting loadTesting processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `loadTesting-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/load-testing.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `loadTesting/${jobId}/output.zip`);
    ...
    logger.error('loadTesting processing failed', { jobId, error });
    throw error;
  }
}
