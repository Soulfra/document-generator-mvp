 ...
 */
export async function processTestGenerator(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting testGenerator processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `testGenerator-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/test-generator.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `testGenerator/${jobId}/output.zip`);
    ...
    logger.error('testGenerator processing failed', { jobId, error });
    throw error;
  }
}
