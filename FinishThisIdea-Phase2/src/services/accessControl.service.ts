...
export async function processAccessControl(
  ...
  try {
    ...
    logger.info('Starting accessControl processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `accessControl-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/access-control.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `accessControl/${jobId}/output.zip`);
    ...
    logger.error('accessControl processing failed', { jobId, error });
    throw error;
  }
}
