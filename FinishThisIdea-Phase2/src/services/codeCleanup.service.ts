 ...
 */
export async function processCodeCleanup(
  jobId: string,
  config: CodeCleanupConfig,
  progressCallback?: (progress: number) => void
): Promise<CodeCleanupResult> {
  ...
  try {
    ...
    logger.info('Starting codeCleanup processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `codeCleanup-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/code-cleanup.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `codeCleanup/${jobId}/output.zip`);
    ...
    logger.error('codeCleanup processing failed', { jobId, error });
    throw error;
  }
}
