 ...
 */
export async function processAiValidator(
  jobId: string,
  config: AiValidatorConfig,
  progressCallback?: (progress: number) => void
): Promise<AiValidatorResult> {
  ...
  try {
    ...
    logger.info('Starting aiValidator processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `aiValidator-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/ai-validator.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `aiValidator/${jobId}/output.zip`);
    ...
    logger.error('aiValidator processing failed', { jobId, error });
    throw error;
  }
}
