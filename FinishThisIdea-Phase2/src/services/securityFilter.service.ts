 ...
 */
export async function processSecurityFilter(
  jobId: string,
  ...
  try {
    ...
    logger.info('Starting securityFilter processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `securityFilter-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/security-filter.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `securityFilter/${jobId}/output.zip`);
    ...
    logger.error('securityFilter processing failed', { jobId, error });
    throw error;
  }
}
