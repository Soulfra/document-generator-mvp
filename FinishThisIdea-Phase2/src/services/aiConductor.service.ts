 ...
 */
export async function processAiConductor(
  jobId: string,
  config: AiConductorConfig,
  progressCallback?: (progress: number) => void
): Promise<AiConductorResult> {
  ...
  try {
    ...
    logger.info('Starting aiConductor processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `aiConductor-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/ai-conductor.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `aiConductor/${jobId}/output.zip`);
    ...
    logger.error('aiConductor processing failed', { jobId, error });
    throw error;
  }
}
