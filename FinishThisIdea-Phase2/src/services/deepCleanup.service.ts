 ...
 */
export async function processDeepCleanup(
  jobId: string,
  config: DeepCleanupConfig,
  progressCallback?: (progress: number) => void
): Promise<DeepCleanupResult> {
  ...
  try {
    ...
    logger.info('Starting deepCleanup processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `deepCleanup-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/deep-cleanup.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `deepCleanup/${jobId}/output.zip`);
    ...
    logger.error('deepCleanup processing failed', { jobId, error });
    throw error;
  }
}
