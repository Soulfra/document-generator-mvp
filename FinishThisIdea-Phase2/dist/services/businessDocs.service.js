...
exports.processBusinessDocs = processBusinessDocs;
 ...
 */
async function processBusinessDocs(jobId, config, progressCallback) {
    ...
    try {
        ...
        logger_1.logger.info('Starting businessDocs processing', { jobId });
        // Download and process
        const inputBuffer = await (0, storage_1.downloadFromS3)(job.inputFileUrl);
        const tempDir = path_1.default.join('/tmp', `businessDocs-${jobId}`);
        ...
        const result = await new Promise((resolve, reject) => {
            const proc = (0, child_process_1.spawn)('python3', [
                ...
                path_1.default.join(__dirname, '../soulfra-scripts/business-docs.py'),
                '--input', inputPath,
                '--output', tempDir,
                '--job-mode'
            ]);
        ...
        const outputUrl = await (0, storage_1.uploadToS3)(outputBuffer, `businessDocs/${jobId}/output.zip`);
        ...
        logger_1.logger.error('businessDocs processing failed', { jobId, error });
        throw error;
    }
}
//# sourceMappingURL=businessDocs.service.js.map