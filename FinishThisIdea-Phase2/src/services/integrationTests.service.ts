 ...
 */
export async function processIntegrationTests(
  jobId: string,
  config: IntegrationTestsConfig,
  progressCallback?: (progress: number) => void
): Promise<IntegrationTestsResult> {
  ...
  try {
    ...
    logger.info('Starting integrationTests processing', { jobId });

    // Download and process
    const inputBuffer = await downloadFromS3(job.inputFileUrl);
    const tempDir = path.join('/tmp', `integrationTests-${jobId}`);
    ...
    const result = await new Promise<any>((resolve, reject) => {
      const proc = spawn('python3', [
        ...
        path.join(__dirname, '../soulfra-scripts/integration-tests.py'),
        '--input', inputPath,
        '--output', tempDir,
        '--job-mode'
      ]);
    ...
    const outputUrl = await uploadToS3(outputBuffer, `integrationTests/${jobId}/output.zip`);
    ...
    logger.error('integrationTests processing failed', { jobId, error });
    throw error;
  }
}
