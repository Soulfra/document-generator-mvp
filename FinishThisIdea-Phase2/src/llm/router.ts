...
export class LLMRouter {
  ...
  private async processWithOllama(task: LLMTask): Promise<LLMResult> {
    ...
    switch (task.type) {
      case 'analyze':
        data = await ollama.analyzeCode(task.input.code);
        confidence = this.calculateAnalysisConfidence(data);
        break;

      case 'cleanup':
        data = await ollama.cleanupCode(task.input.code, task.input.language);
  ...
  private async processWithClaude(task: LLMTask): Promise<LLMResult> {
    ...
    switch (task.type) {
      case 'analyze':
        const analysisResult = await claude.analyzeCode(task.input.code);
